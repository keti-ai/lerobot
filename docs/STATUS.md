...
**마지막 갱신:** 2026-06-11 (🟡 **FLAG — banana grasp "가끔 한 번"(occasional, 불안정, 풀린 것 아님)** (D07r, compile+h20). **정직한 현실(사용자)**: deploy 튜닝으로 never→occasional 일 뿐 — ①성공률 매우 낮음 ②움직임 여전히 거침(톡톡 spike 잔여, H=30 한계) ③**정책 grasp 각도/방향 커버리지 좁음=모르는 자세 많음(데이터 천장)**. **방법 총망라 = `audits/openarm_folding/d42_methods_summary_20260611.md`**. best config: 서버 compile+horizon20 pid797370 8081, profile diag_handover_grip(arm25/j4-5=65/j6-7=40/grip40, interp3). deploy 레버(RTC window/cap/aggregation) 대체로 소진 → **70% 본 해법은 data 다양성+finetune 일 공산**. 추후: 안정성·smoothness·grasp coverage(data)·N=20.)
