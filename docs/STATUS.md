...
**마지막 갱신:** 2026-06-11 (🟡 **FLAG — banana grasp "가끔 한 번"(occasional, 불안정, 풀린 것 아님)** (D07r, compile+h20). 사실: ①성공률 매우 낮음 ②움직임 여전히 거침(톡톡 spike 잔여, H=30 한계) ③**grasp 각도/방향 커버리지 미지(측정된 적 없음, 사용자 관찰)**. **방법 총망라+열린 옵션 풀 = `audits/openarm_folding/d42_methods_summary_20260611.md` §5/§5.5**. best config: 서버 compile+horizon20 pid797370 8081, profile diag_handover_grip(arm25/j4-5=65/j6-7=40/grip40, interp3). **다음 방향 단정 안 함** — 옵션 풀: (가)커버리지 규명 먼저 (나)deploy 잔여 트릭(#6 guidance β↑, #8 g0.7, latest_only WC1/WD1, LS0 homing, LS1/2, ACG) (다)재학습 시 #9 finetune+#10 training-time RTC 콤보. 측정 기반으로 추후 같이 결정. **+TS1 확정(2026-06-12, 🟢 good result)**: 게인/속도/프로파일 스윕(D07s~v) 완료 — **D07v before/after 로 TS1 ≫ interp×3 확정** (interp3 의 cmd qvel ±200-400°/s 스파이크="덜컥"의 실체 → TS1 100Hz trapezoidal 로 ±120 envelope, 스파이크 소멸. PNG 증거 `results/ts1_tuning_20260612/`). **deploy stack v2 확정: 서버 compile+h20 + client TS1 100Hz + kp [312×4,43.2,55.8,45,32.5](손목×1.8/나머지×1.3) + v120 = profile `traj_trap_100_v120_kp18x13x`**. audit `ts1_tuning_results_20260612.md`. 재정리된 다음: ①stack v2 로 full-task 반복(성공률 변화?) ②오르면 N=20 ③안 오르면 grasp 커버리지 규명→data/finetune.)
