...
**마지막 갱신:** 2026-06-11 (🟢 **FLAG 고정 — 첫 banana grasp 성공** (D07r, compile+h20). trajectory 첫 실질 성공. **방법 총망라 보고서 = `audits/openarm_folding/d42_methods_summary_20260611.md`** (서빙/RTC window/cap/진단 전부). frozen config: 서버 compile+horizon20 pid797370 8081(window 9≤20≤21, forward210ms), profile diag_handover_grip(arm25/j4-5=65/j6-7=40/grip40, interp3, weighted_average). 핵심 = RTC window 복원(compile d↓+horizon20) + per-joint cap(손목 top-down) + gentle gripper. 남은 작업(추후): 안정성(겨우1회·spike 잔여), compile parity diff 0.19 영향, 객체 일반화(toothpaste/cup), latest_only A/B, **operator N=20(70%)**.)
