...
**마지막 갱신:** 2026-06-10 (**★ 현상 확정: 최근 회귀 진범 = host CPU starvation (software 아님)**. CAN 무결점(PEAK PCAN-USB Pro FD, ip-stats bus-err/dropped/missed 0) → "joint_2 packet drop" 은 전기 아닌 **damiao 10ms read timeout(damiao.py:63)을 호스트가 놓침**. 호스트 load avg **13.68**(trial 전인데도), 주범 **rustdesk 13.4%+Xwayland/i915/flush**. 부하→jitter→control loop 16fps + 카메라 3대 동기 read(openarm_follower.py:277-279)가 모터 창 밀어냄→joint_2 stale→**top-down 붕괴+톡톡+grasp 흔들림**. **D07f~o 전부 이 부하 위 = 오염**(compile/grasp/톡톡 결론 잠정). 계획: 랩탑 부하제거+chrt RT priority+bf16 서버 → D07p 클린 baseline 재확립 → 재판정. plan ~/.claude/plans/quizzical §상단)
