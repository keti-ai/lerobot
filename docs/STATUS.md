...
**마지막 갱신:** 2026-06-11 (**★ 현상 재확정 (plan §상단 승인)**: 자잘한 transient 정리 — joint_2 packet drop/handshake = 단순 24V 전원 빠짐(재연결로 해결, minor), host load(13.68→3.59)는 fps 거의 무변(I/O-bound, 주원인 아님). **진짜 persistent 원인 = RTC 실행층이 깨진 구간(`d>s`)에서 돎**: live d≈15-27 > execution_horizon s=10 → boundary 톡톡 + 그 위 weighted_average(0.3old+0.7new)/interp×3 가 정책의 손목 회전 씻음(grasp slip). **사용자 지적+스터디(d≤s≤H−d): horizon 길게=부드러움 → K11 의 s 20→10 축소가 착시였음, s 키워야.** 계획: ①α'' H+live d 확정 →②horizon s 10→~15-20 + compile d≈9 (window 복원, 1차 smoothness) →③오프라인 blending 댐핑 진단 →④no-blend(latest_only) profile →⑤operator A/B. 학습0. plan ~/.claude/plans/quizzical §상단. **WB1 완료(32d2b0b2)**: H=30(노트 50 가정 틀림), 측정 d≈14(server-side), **horizon s 10→15** → window 14≤15≤16 충족(ok_rate 1.0), **compile SKIP — bf16 로 충분(compile/OOM 우회)**. 서버 bf16+h15 pid604002 8081. **D07q 결과: grasp 잘 됨✓(per-joint cap+window 로 손목 회전 해결, persistent core 마일스톤), 단 톡톡 "아주 조금"만↓ → 아직 퉁 쳐버림.** 진단: horizon15 가 steady window(d14≤s15) 충족했으나 **live 네트워크 spike(max_net 957ms≈순간 d29)** 가 순간 window 깸 → 톡톡. H=30 이라 horizon 으론 spike 못 덮음(s≤16). → **처방=compile(forward396→228=steady d14→9 → s15 안 ~6step spike 여유)**. OOM=MB2 pad 해결, top-down 회귀=joint_2 confound debunked → 클린서 parity 재확인. **사용자 결정: D07r = compile on(d≈9) + horizon s 15→20** (compile 이 창을 [9,21]로 넓혀 horizon 천장 16→21 상승 → 더 긴 horizon=더 부드러움 + spike 여유). s=20≤H−d=21 ✓(상한 1step 여유, 톡톡 재발 시 18로). a6000 compile+h20 재기동 → operator D07r → 톡톡 퉁 해소 확인 → L4 N=20. **WB2 완료(5b3ca18c)**: compile+h20, window 9≤20≤21 ok_rate1.0, forward 210ms, mem9932 stable, recompile0, 정책 재사용 guard(setup 0.876ms). **⚠️ parity diff 0.19 max(mean0.0165, joint_2 0.078) = K14/15(0.006-0.012)의 ~20-30배** — compile drift or MB2 pad 로직. D07r 가 grasp/top-down 영향 판정. **깨끗한 fallback 無**: bf16 은 d14 라 s≤16 → s20 불가. compile+s20(부드러움,diff0.19) vs bf16+s15(parity,톡톡) 택1. 서버 pid797370 compile+h20 8081.)
